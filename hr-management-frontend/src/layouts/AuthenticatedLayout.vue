...
<template>
    <div v-if="auth" class="min-h-screen bg-white">
        ...
        <aside
            ...
            :class="[sidebarWidthClass, { '-translate-x-full': !sidebarOpen }]">
            <!-- Logo Section with Collapse Toggle -->
            <div
                class="flex flex-col items-center py-6 border-b border-white bg-green-700 flex-shrink-0">
                <router-link
                    to="/dashboard"
            ...
            <div
                v-if="!sidebarCollapsed"
                class="hidden lg:block absolute top-6 -right-3 z-40">
                <button
            ...
            <div
                v-if="sidebarCollapsed"
                class="hidden lg:flex justify-center py-3">
                <button
                    @click="toggleCollapse"
            ...
            <nav class="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
                ...
                <div class="relative group">
                    ...
                    <div
                        v-if="sidebarCollapsed"
                        class="absolute bg-gray-900 text-white px-2 py-1 rounded text-sm whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none z-[100] shadow-lg"
                        style="
                            left: 100%;
                            margin-left: 8px;
                            top: 50%;
                        ...
                        Dashboard
                        <div
                            class="absolute right-full top-1/2 transform -translate-y-1/2 border-4 border-transparent border-r-gray-900"></div>
                    </div>
                </div>
                ...
                <div class="relative group">
                    <router-link
                        ...
                        :title="sidebarCollapsed ? 'All Products' : ''"
                        @click="closeSidebar">
                        <svg
                            class="w-5 h-5 transition-transform duration-300 group-hover:scale-110"
                            :class="{ 'mr-3': !sidebarCollapsed }"
            ...
            <div
                class="p-4 border-none flex-shrink-0 relative"
                @mouseenter="dropdownOpen = true"
                @mouseleave="dropdownOpen = false">
                <!-- User Email Button -->
                <button
                    class="flex items-center w-full py-3 text-gray-800 bg-white rounded-lg hover:bg-gray-100 transition-colors duration-200"
                    :class="sidebarCollapsed ? 'justify-center px-0' : 'px-4'"
                    :title="sidebarCollapsed ? store.userData.email : ''"
                    @click="toggleDropdown">
                    <svg
                        class="w-5 h-5"
                        :class="{ 'mr-4': !sidebarCollapsed }"
                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24">
                        <path
                ...
                <div
                    v-if="dropdownOpen"
                    class="absolute bg-white rounded-lg shadow-xl border border-gray-200 overflow-hidden z-50"
                    :class="sidebarCollapsed ? 'left-2 right-2' : 'left-4 right-4'"
                    style="bottom: 100%;">
                    <button
                        @click="submitLogout"
                        class="flex items-center w-full py-3 text-gray-700 hover:bg-red-50 hover:text-red-600 transition-colors duration-200"
                        :class="sidebarCollapsed ? 'justify-center px-0' : 'px-4'"
